...
import paydunya from "../config/paydunya.js";

const router = express.Router();

// Initialisation NotchPay avec la clé depuis .env
// const notchpay = NotchPay(process.env.NOTCHPAY_API_KEY);
...
router.post("/pay", async (req, res) => {
  try {
    ...
    invoice.setDescription("Paiement Cesame");

    // 🔁 URLs
    invoice.setReturnUrl("http://localhost:3000/success");
    invoice.setCancelUrl("http://localhost:3000/cancel");
    invoice.setCallbackUrl("http://localhost:5000/api/webhook");

    // 👤 Client
    invoice.addCustomerInfo({
      name,
      email
    });
...
router.post("/webhook", (req, res) => {
  ...
  // ⚠️ Vérification basique (améliorable)
  if (data.status === "completed") {

    // 👉 Exemple : update user / paiement
